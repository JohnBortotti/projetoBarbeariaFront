<template>
  <div class="container">
    ...
    <div class="row justify-content-sm-center">
      <form class="w-25" @submit.prevent="postServico(service_id, client_id, servico, status, data, horario)">
        <h3 class="mb-5 text-center">Atualizar Serviço</h3>
        <div class="form-group">
          <label>Servico ID</label>
          <input
            class="form-control"
            type="number"
            v-model="service_id"
            placeholder="insira o id do servico"
            required
          />
        </div>
        <div class="form-group">
...
export default {
  ...
  methods: {
    postServico: function(service_id, client_id, servico, status, data, horario) {
      fetch(`http://localhost:8000/api/service/${service_id}`, {
        method: "PATCH",
        headers: {
